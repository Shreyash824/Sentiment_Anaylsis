...
<style>
    ...
    .empty {
        text-align: center; padding: 60px 20px; border: 2px dashed #c3c9e5;
        border-radius: 16px; background: #f8f9fc; color: #555;
    }
    .empty h3 { margin: 10px 0 6px; color: #444; }
</style>
"""
...
def main():
    st.markdown(
        ...
        <div class="hero">
            ...
            <b>positive</b>, <b>negative</b> or <b>neutral</b> - works for movies,
    ...
    if df is None:
        st.markdown(
            ...
            <div class="empty">
                ...
                <p>Use the sidebar to upload a CSV or Excel file containing reviews.
                <br>Your file can hold movie reviews, product feedback, service ratings
                or any other text.</p>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    if not run:
        st.info("Press **Run sentiment analysis** to segment the reviews.")
        st.stop()

    prog = st.progress(0.0, text="Scoring reviews...")

    def tick(fraction):
        prog.progress(fraction, text=f"Scoring reviews... {fraction*100:.0f}%")
    ...
    tab_overview, tab_reviews, tab_segments = st.tabs(
        ["Overview", "All Reviews", "By Segment"])
    ...
    with tab_reviews:
        search = st.text_input("Search reviews", placeholder="Type a keyword to filter...")
    ...
    with tab_segments:
        for seg, color in (("positive", "#2ecc71"), ("neutral", "#f39c12"), ("negative", "#e74c3c")):
            sub = out[out["sentiment"] == seg]
            with st.expander(
                f"{seg.upper()} - {len(sub)} reviews ({pct_map[seg]:.1f}%)",
