...
def decode_bytes(raw, enc=None):
    candidates = [c for c in (enc, "utf-8-sig", "utf-8", "latin-1", "cp1252") if c]
...
def analyze_dataframe(df, text_col, progress_cb=None, keep_extra_cols=False):
    analyzer = SentimentIntensityAnalyzer()
    base = df[[text_col]].copy() if not keep_extra_cols else df.copy()
    cleaned = []
    scores = []
    sentiments = []
    total = len(df)
    for i, text in enumerate(df[text_col]):
        ...
        if progress_cb is not None and i % max(1, total // 20) == 0:
            progress_cb(i / total)
    base["cleaned_text"] = cleaned
    base["sentiment_score"] = scores
    base["sentiment"] = sentiments
    out = base
